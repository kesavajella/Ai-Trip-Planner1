...
@GetMapping("/trips")
    ...
    public ResponseEntity<?> saveTrip(@PathVariable String id, HttpSession session) {
        ...
        tripService.saveTripToSavedPlaces(trip);
    ...
    public ResponseEntity<?> generateTrip(@RequestBody Map<String, Object> body, HttpSession session) {
        try {
            ...
            ItineraryGeneratorService.ItineraryGenerationResult generationResult = itineraryGenerator.generateItineraryWithRawResponse(request);
            ItineraryData itinerary = generationResult.itinerary();
            TripPlanResponse rawGeminiResponse = generationResult.rawResponse();
            ...
            String rawGeminiJson = null;
            ObjectMapper mapper = new ObjectMapper();
            if (rawGeminiResponse != null) {
                try {
                    rawGeminiJson = mapper.writeValueAsString(rawGeminiResponse);
                } catch (Exception e) {
                    log.warn("Failed to serialize raw Gemini response: {}", e.getMessage());
                }
            }

            Trip savedTrip = saveTripFromItinerary(city, source, startDate, countryCode, countryName, numberOfDays, budget, travelers, interestsList, accommodationPreference, itinerary, userId, itinerary.getDestinationCurrencyCode(), rawGeminiJson);
            ...
            try {
                ...
                payload.put("source", itinerary.getSource());
                payload.put("startDate", itinerary.getStartDate());
            ...
            log.warn("⚠️ [Gemini Rate Limit / Quota Exceeded]: {}", e.getMessage());
            return ResponseEntity.status(HttpStatus.TOO_MANY_REQUESTS)
                    .body(Map.of(
                            "error", "Daily limit reached, please try again shortly",
                            "details", "The AI service rate limit has been reached. Please wait a moment before submitting a new trip plan request."
                    ));
        } catch (Exception e) {
    ...
    private Trip saveTripFromItinerary(String city, String source, String startDate, String countryCode, String countryName, int days, String budget, String travelers, List<String> interests, String accommodationPreference, ItineraryData itinerary, String userId, String destinationCurrencyCode, String rawGeminiJson) {
        Trip trip = new Trip();

        String[] cityParts = city.split(",");
        trip.setDestination(cityParts[0].trim());
        trip.setSource(source != null && !source.isBlank() ? source.trim() : null);
        ...
        trip.setRawGeminiResponse(rawGeminiJson);
        trip.setStatus("upcoming");
        trip.setSaved(true);
        trip.setStartDate(startDate != null && !startDate.isBlank() ? startDate.trim() : LocalDateTime.now().format(DateTimeFormatter.ofPattern("yyyy-MM-dd")));
        trip.setCreatedAt(LocalDateTime.now());
         ...
         tripService.saveTripToSavedPlaces(savedTrip);
         return savedTrip;
     }
    ...
    private String mapItineraryToJson(ItineraryData itinerary) {
        try {
            ...
            itineraryMap.put("source", itinerary.getSource());
            itineraryMap.put("startDate", itinerary.getStartDate());
